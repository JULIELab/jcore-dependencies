...
public class BasicChunk {
    ...
    public void mergeNP() {
        ...
        while (i <chunkList.size()) {
            ...
            if (prev != null && prev.type.equals("NP") && curr.type.equals("NP")) {
                ...
                continue;
            } else if (curr.type.equals("ADVP") && curr.pros.isEmpty() && curr.trigs.isEmpty()&& i< chunkList.size()-1) {
                next = chunkList.get(i + 1);
                if (prev != null && prev.txt.equals(",") && next.txt.equals(",")) {
                    //System.out.println("---> before remove advp: "+toString());
                    chunkList.remove(prev);
                    chunkList.remove(curr);
                    chunkList.remove(next);
                    prev = null;
                    //System.out.println("---> after remove advp: "+toString());
                    continue;
                }
            }
            i++;
    ...
    public void findCoordinator(List<Chunk> ls) {
        ...
        while (i < ls.size() - 2) {
            ...
            if (curr.type.equals("NP")) {
                int j = i + 1;
                if (curr.txt.contains(",")) { // 14.10.2011 : add this condition since in some cases, the parser has already merged co-ordinative chunks
                    in_loop = true;
                }
                while (j < ls.size() - 1) {
                    next = ls.get(j);
                    ...
                    if (next.txt.equals(",") && next1.type.equals("NP")) {// [,] [NP]
                        j += 2;
                        in_loop = true;
                        continue;
                    } else if (next.txt.equals(",")) {//[,] -> [CC] [NP]|[appo][NP] 
                        if (next1.type.equals("O") && ccMap.contains(next1.txt) && in_loop) { // expect NP
                            if (j < ls.size() - 2) {
                        ...
                        } else if (appoMap.contains(next1.txt)) { // not in loop, check for apposition
                            if (j < ls.size() - 2) {
                                Chunk tmp = ls.get(j + 2);
                                if (tmp.type.equals("NP")) {
                                    idx =j+2;
                        ...
                        if (!in_loop) {
                            if (j < ls.size() - 2) {
                                Chunk tmp = ls.get(j + 2);
                                if (tmp.txt.equals("of")) { // ignore this case, second NP might related to 'of'
                                    found = false;
                                }
                            }
                        }
                ...
                if (found) {// [NP],[NP],...,[and][NP]
                    ...
                    curr.is_merged = true;
                    continue;// keep the same chunk, migh be has another co-ordination
                }
            }
            i++;
        }
    ...
    /**
     * Some known patterns/rules:
     * 1. [PRO1 TRIG] of [PRO2]: 
     * >> if trg is a reg event: -> TRG|PRO2|PRO1 (reg event)
     * >> else -> TRG|PRO1 (simple event)
     * 
     * 2. [TRG1 TRG2 ] of [PRO] 
     * if TRG1 is a reg event then:
     * Event: TRG2|PRO  and TRG1|[TRG2|PRO]
     * 
     * 3. [PRO1-TRG1 TRG2] of [PRO2]
     * if TRG1 is a reg event then:
     * Event: TRG2|PRO  and TRG1|[TRG2|PRO2]|PRO1
     * 4. [TRG1] of [TRG2] of [PRO]
     * if TRG1 is reg event then:
     * Event: TRG2|PRO and TRG1|[TRG2|PRO]
     */
    /**
     * Operation:
     * 1. Check in_chunk: trg and pro in the same chunk, pro preceeds trg, trig's POS is a NN
     * if trg's POS is VBx and trg is a reg event -> PRO is used as cause
     * 2. Find connection: NP prep NP : if trig is NP and not (in_chunk) use known list of preps to
     * connect with another NP
     * 3. Check role: if there is more than one trig in chunk, check relationship (equals, dependent) between trgs
     * two trig are equal if they connected by CC (i.e. and,or) and has the same POS
     * 4. Divide into simple/binding/reg event to make extraction process simpler
     * 5. How to store extracted events? How to assign ID?-> List of extracted event, reference via object ID
     */
    @Override
