...
public class BioSemApplication {
	...
	static final ArrayList<String> SET_NAMES = new ArrayList<String>(){{add("train"); add("test"); add("devel");}}; 
	...
	private void readCommandLineArgs(String[] args) {
		...
		final CmdLineParser.Option textFolderOption = paramParser.addStringOption(
				'i', "input");
		final CmdLineParser.Option dbFolderOption = paramParser.addStringOption(
				'o', "output");
		...
		try {
			...
			/** ----- Modus ----- **/
			param = paramParser.getOptionValue(modusOption);
			if (param != null) {
				modus = (String) param;
				if (!(getModus().equalsIgnoreCase("complete")
						|| getModus().equalsIgnoreCase("data-preparation")
						|| getModus().equalsIgnoreCase("trigger-learn")
						|| getModus().equalsIgnoreCase("pattern-learn")
			...
			/** ----- Input Folder ----- **/
			param = paramParser.getOptionValue(textFolderOption);
			if (param != null) {
			...
			/** ----- DataBase Folder ----- **/
			param = paramParser.getOptionValue(dbFolderOption);
			if (param != null) {
			...
			/** ----- Data Set ----- **/
			param = paramParser.getOptionValue(setOption);
			if (param != null) {
				set = (String) param;
				if (!(getSet().equalsIgnoreCase("train")
						|| getSet().equalsIgnoreCase("test")
						|| getSet().equalsIgnoreCase("devel")
						|| getSet().equalsIgnoreCase("mix"))) {
			...
			/** ----- Mix Train and Devel ----- **/
			param = paramParser.getOptionValue(mixOption);
			if (param != null) {
			...
			/** ----- Destination ----- **/
			param = paramParser.getOptionValue(destOption);
			if (param != null) {
				destination = (String) param;
				if (!SET_NAMES.contains(getDestination().toLowerCase())) {
					System.out.println(NEWLINE+"\""+getDestination()+"\" is no valid destination; choose: \"train\","
	...
	private void learnTrigger(String src, String dest) {
		...
		TriggerLearner learner = new TriggerLearner(dbsrc, dbdest); // store all triggers (both train and dev) into train DB
