 ...
 * <li>clear blockers, clear distances, clear datasets
 * <li>show blockers, show distances, show datasets
 * <p>
 * <li>compute: compute pairwise distances for using all declared blockers, distances, datasets
...
public class MatchExptScript
{
	...
	{ 
		...
		else if (what.equals("distances")) showList("distances", distanceNames); 
		else if (what.equals("all")) {
			show("blockers"); show("datasets"); show("distances"); 
		} else {
	...
	{
		distances.add( DistanceFactory.build( distanceClass ));
		distanceNames.add( distanceClass );
		expt = null;
	}
	...
	public void compute() 
	{
		...
		expt = new MatchExpt[blockers.size()][distances.size()][datasets.size()];
		for (int i=0; i<blockers.size(); i++) {
			Blocker blocker = (Blocker)blockers.get(i);
			for (int j=0; j<distances.size(); j++) {
				StringDistance distance = (StringDistance)distances.get(j);
	...
	public void table(String what) throws NoSuchMethodException,IllegalAccessException,InvocationTargetException
	{
		...
		for (int i=0; i<blockerNames.size(); i++) {
			...
			System.out.print("           ");
			for (int j=0;  j<distanceNames.size(); j++) {
				String s = (String)distanceNames.get(j);
				if (s.length()>11) s = s.substring(0,11);
				System.out.print( nfmt.sprintf(s) );
			}
			System.out.println();
			double[] average = new double[distanceNames.size()];
	...
	public void precisionRecall()
	{
		...
		for (int i=0; i<blockerNames.size(); i++) {
			...
			for (int j=0; j<distanceNames.size(); j++) {
				String distance = (String)distanceNames.get(j);
	...
	public void restore(String file) throws IOException,FileNotFoundException,ClassNotFoundException
	{
		...
		distanceNames = (List)ois.readObject();
