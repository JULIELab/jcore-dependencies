...
public class TestNaiveBayes {
    ...
    public void testNonTrained() {
        ...
        Classifier c = new NaiveBayes(new Noop(fdict, ldict), prior, new Multinomial[] { sports, politics });

        Instance inst = c.getInstancePipe().instanceFrom(
                new Instance(new FeatureVector(fdict, new Object[] { "speech", "win" }, new double[] { 1, 1 }),
                        ldict.lookupLabel("politics"), null, null));
        ...
        assertTrue(cf.getLabeling().getBestLabel() == ldict.lookupLabel("politics"));
        assertTrue(cf.getLabeling().getBestValue() > 0.6);
    }

    @Test
    public void testStringTrained() {
        String[] africaTraining = new String[] { "on the plains of africa the lions roar",
                "in swahili ngoma means to dance", "nelson mandela became president of south africa",
                "the saraha dessert is expanding" };
        String[] asiaTraining = new String[] { "panda bears eat bamboo",
                "china's one child policy has resulted in a surplus of boys", "tigers live in the jungle" };

        InstanceList instances = new InstanceList(
                new SerialPipes(new Pipe[] { new Target2Label(), new CharSequence2TokenSequence(),
        ...
        assertTrue(cf.getLabeling().getBestLabel() == ((LabelAlphabet) instances.getTargetAlphabet())
                .lookupLabel("africa"));
    }
    ...
    public void testIncrementallyTrainedGrowingAlphabets() {
        ...
        String[] args = new String[] { "src/cc/mallet/classify/tests/NaiveBayesData/learn/a",
        ...
        new SerialPipes(new Pipe[] { new Target2Label(), new Input2CharSequence(),
        ...
        // incrementally train...
        String[] t2directories = { "src/cc/mallet/classify/tests/NaiveBayesData/learn/b" };
