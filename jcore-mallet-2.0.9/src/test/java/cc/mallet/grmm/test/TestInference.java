...
public class TestInference {
    ...
    final public Class[] algorithms = { BruteForceInferencer.class, VariableElimination.class,
            JunctionTreeInferencer.class, };

    final public Class[] appxAlgs = { TRP.class, LoopyBP.class, };

    // only used for logJoint test for now
    final public Class[] allAlgs = {
            // BruteForceInferencer.class,
            JunctionTreeInferencer.class, TRP.class,
            // VariableElimination.class,
            LoopyBP.class, };

    final public Class[] treeAlgs = { TreeBP.class, };

    static List modelsList;
    static UndirectedModel[] models;
    static FactorGraph[] trees;
    static Factor[][] treeMargs;
    ...
    private static UndirectedModel createTriangle() {
        ...
        double[][] pots = new double[][] { { 0.2, 0.8, 0.1, 0.9 }, { 0.7, 0.3, 0.5, 0.5 }, { 0.6, 0.4, 0.8, 0.2 },
                { 0.35, 0.65 } };
    ...
    private static TableFactor randomEdgePotential(Random r, Variable v1, Variable v2) {
        ...
        return new TableFactor(new Variable[] { v1, v2 }, phi);
    }
    ...
    private static TableFactor randomNodePotential(Random r, Variable v) {
        ...
        return new TableFactor(new Variable[] { v }, phi);
    }
    ...
    private static UndirectedModel createRandomTree(int nnodes, int maxOutcomes, Random r) {
    ...
    public static List createTestModels() {
        ...
        // inferencer on them.
        FactorGraph[] mdls = new FactorGraph[] { createTriangle(), createChainGraph(), createRandomGraph(3, 2, r),
                createRandomGraph(3, 3, r), createRandomGraph(6, 3, r), createRandomGraph(8, 2, r),
                createRandomGrid(3, 2, 4, r), createRandomGrid(4, 3, 2, r), };
            ...
            for (int mdl = 0; mdl < models.length; mdl++) {
                ...
                while (it.hasNext()) {
                    try {
                        ...
                        logger.finest("logJoint: " + inf1 + " " + inf2 + "  Model " + mdl + "  Assn: " + assn
                                + "  INF1: " + joint1 + "\n" + "  INF2: " + joint2 + "\n");

                        assertTrue(
                                "logJoint not equal btwn " + GeneralUtils.classShortName(inf1) + " " + " and "
                                        + GeneralUtils.classShortName(inf2) + "\n" + "  Model " + mdl + "\n"
                                        + "  INF1: " + joint1 + "\n" + "  INF2: " + joint2 + "\n",
    ...
    public void testMarginals() throws Exception {
        ...
        /*
         * Now, make sure the exact marginals are consistent for the same model.
         */
        for (int mdl = 0; mdl < models.length; mdl++) {
        ...
        for (int appxIdx = 0; appxIdx < appxInferencers.length; appxIdx++) {
            ...
            for (int mdl = 0; mdl < models.length; mdl++) {
                ...
                for (Iterator it = models[mdl].variablesSet().iterator(); it.hasNext(); vrt++) {
    ...
    private Factor[] collectAllMarginals(FactorGraph mdl, Inferencer alg) {
        ...
        for (Iterator it = mdl.variablesSet().iterator(); it.hasNext(); vrt++) {
            Variable var = (Variable) it.next();
            try {
                collector[vrt] = alg.lookupMarginal(var);
                assert collector[vrt] != null : "Query returned null for model " + mdl + " vertex " + var + " alg "
                        + alg;
            } catch (UnsupportedOperationException e) {
    ...
    public void testQuery() throws Exception {
        ...
        for (int mdlIdx = 0; mdlIdx < models.length; mdlIdx++) {
            ...
            for (int algIdx = 0; algIdx < appxAlgs.length; algIdx++) {
                ...
                if (alg instanceof TRP) {
                    continue; // trp can't handle disconnected models, which arise during query()
                }
    ...
    private void testSerializationForAlg(Inferencer alg) throws IOException, ClassNotFoundException {
        for (int mdlIdx = 0; mdlIdx < models.length; mdlIdx++) {
            ...
            compareMarginals("Error comparing marginals after serialzation on model " + mdl, pre, post2);
        }
    }
    ...
    private static UndirectedModel createJtChain() {
        int numNodes = 4;
        ...
        Factor[] pots = new TableFactor[] {
                new TableFactor(new Variable[] { nodes[0], nodes[1] }, new double[] { 1, 2, 5, 4 }),
                new TableFactor(new Variable[] { nodes[1], nodes[2] }, new double[] { 4, 2, 4, 1 }),
                new TableFactor(new Variable[] { nodes[2], nodes[3] }, new double[] { 7, 3, 6, 9 }), };
    ...
    private static void createTestTrees() {
        Random r = new Random(185);
        trees = new FactorGraph[] { RandomGraphs.createUniformChain(2), RandomGraphs.createUniformChain(4),
                createJtChain(), createRandomGrid(5, 1, 3, r), createRandomGrid(6, 1, 2, r), createRandomTree(10, 2, r),
                createRandomTree(10, 2, r), createRandomTree(8, 3, r), createRandomTree(8, 3, r), };
        modelsList.addAll(Arrays.asList(trees));
    }

    private static void computeTestTreeMargs() {
    ...
    private static String[] treeStrs = new String[] {
            "<TREE>" + "  <VAR NAME='V0'>" + "    <FACTOR VARS='V0 V1'>" + "      <VAR NAME='V1'/>" + "    </FACTOR>"
                    + "    <FACTOR VARS='V0 V2'>" + "      <VAR NAME='V2'/>" + "    </FACTOR>" + "  </VAR>" + "</TREE>",
            "<TREE>" + "  <VAR NAME='V1'>" + "    <FACTOR VARS='V0 V1'>" + "      <VAR NAME='V0'/>" + "    </FACTOR>"
                    + "    <FACTOR VARS='V1 V2'>" + "      <VAR NAME='V2'/>" + "    </FACTOR>" + "  </VAR>" + "</TREE>",
            "<TREE>" + "  <VAR NAME='V0'>" + "    <FACTOR VARS='V0 V1'>" + "      <VAR NAME='V1'>"
                    + "  <FACTOR VARS='V1 V2'>" + "    <VAR NAME='V2'/>" + "  </FACTOR>" + "</VAR>" + "    </FACTOR>"
                    + "  </VAR>" + "</TREE>",
            "<TREE>" + "  <VAR NAME='V2'>" + "    <FACTOR VARS='V2 V1'>" + "      <VAR NAME='V1'/>" + "    </FACTOR>"
                    + "    <FACTOR VARS='V0 V2'>" + "      <VAR NAME='V0'/>" + "    </FACTOR>" + "  </VAR>"
                    + "</TREE>", };
    ...
    public void testTrpViterbiEquiv() {
        for (int mdlIdx = 0; mdlIdx < trees.length; mdlIdx++) {
            ...
            for (Iterator it = mdl.variablesIterator(); it.hasNext();) {
                ...
                maxPotTrp.normalize();
                assertTrue(
                        "TRP 1 iter maxprod propagation not the same as plain maxProd!\n" + "Trp "
                                + maxPotTrp.dumpToString() + "\n Plain maxprod " + maxPotBp.dumpToString(),
        ...
        for (int mdlIdx = 0; mdlIdx < models.length; mdlIdx++) {
            ...
            for (Iterator it = mdl.variablesIterator(); it.hasNext();) {
                ...
                assertTrue("Maximization failed on model " + mdlIdx + " ! Normalized returns:\n" + maxPot.dumpToString()
                        + "\nTrue: " + trueMaxPot.dumpToString(), maxPot.almostEquals(trueMaxPot, 0.01));
            }
        }
    ...
    /*
     * public void testMM() throws Exception { testQuery(); testTreeViterbi(); testTrpViterbiEquiv();
     * testTrpViterbiEquiv2(); testMaxMarginals(); }
     */
    ...
    public void testBeliefPropagation() {
        for (int mdlIdx = 0; mdlIdx < trees.length; mdlIdx++) {
            ...
            for (Iterator it = mdl.variablesIterator(); it.hasNext();) {
                ...
                try {
                    assertTrue(
                            "Test failed on graph " + mdlIdx + " vertex " + var + "\n" + "Model: " + mdl
    ...
    public void testDirectedJt() {
        ...
        compareMarginals("Error comparing junction tree to brute on directed model!", bn, brute, jt);
    }
    ...
    public static void setUp() {
        ...
        createTestTrees();
        models = (UndirectedModel[]) modelsList.toArray(new UndirectedModel[] {});
    ...
    /*
     * TODO: Not sure how to test this anymore. // Test multiplication of potentials where variables are in // a
     * different order public void testMultiplication2 () { Variable[] vars = new Variable[] { new Variable (2), new
     * Variable (2), }; double[] probs1 = new double[] { 2, 4, 1, 6 }; double[] probs2a = new double[] { 3, 7, 6, 5 };
     * double[] probs2b = new double[] { 3, 6, 7, 5 };
     *
     * MultinomialPotential ptl1a = new MultinomialPotential (vars, probs1); MultinomialPotential ptl1b = new
     * MultinomialPotential (vars, probs1); MultinomialPotential ptl2a = new MultinomialPotential (vars, probs2a);
     *
     * Variable[] vars2 = new Variable[] { vars[1], vars[0], }; MultinomialPotential ptl2b = new MultinomialPotential
     * (vars2, probs2b);
     *
     * ptl1a.multiplyBy (ptl2a); ptl1b.multiplyBy (ptl2b);
     *
    ...
    public void testLogNormalize() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            norm1.normalize();
            norm2.normalize();
            assertTrue("LogNormalize failed! Correct: " + norm2 + " Log-normed: " + norm1, norm1.almostEquals(norm2));
        }
    }
    ...
    public void testLoopyCaching() {
        ...
        assertTrue("Huh? Original potential:" + origPtl + "After: " + sndPtl, origPtl.almostEquals(sndPtl, 1e-4));
    ...
    /*
     * public void testBpDualEdgeFactor () { Variable[] vars = new Variable[] { new Variable (2), new Variable (2), new
     * Variable (2), new Variable (2), };
     *
     * Random r = new Random (); Factor tbl1 = createEdgePtl (vars[0], vars[1], r); Factor tbl2a = createEdgePtl
     * (vars[1], vars[2], r); Factor tbl2b = createEdgePtl (vars[1], vars[2], r); Factor tbl3 = createEdgePtl (vars[2],
     * vars[3], r);
     *
     * FactorGraph fg = new FactorGraph (vars); fg.addFactor (tbl1); fg.addFactor (tbl2a); fg.addFactor (tbl2b);
     * fg.addFactor (tbl3);
     *
     * Inferencer inf = new TRP (); inf.computeMarginals (fg);
     *
     * VarSet vs = tbl2a.varSet (); Factor marg1 = inf.lookupMarginal (vs);
     *
     * Factor prod = TableFactor.multiplyAll (fg.factors ()); Factor marg2 = prod.marginalize (vs); marg2.normalize ();
     *
     * assertTrue ("Factors not equal!  BP: "+marg1.dumpToString ()+"\n EXACT: "+marg2.dumpToString (),
     * marg1.almostEquals (marg2)); }
     */
