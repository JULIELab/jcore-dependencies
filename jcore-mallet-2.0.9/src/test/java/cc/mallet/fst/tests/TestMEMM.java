...
public class TestMEMM {
        ...
        for (int i = 0; i < numStates; i++) {
            for (int j = 0; j < numStates; j++) {
                for (int k = 0; k < inputVocabSize; k++) {
                    ...
                    System.out.println("parameters " + i + " " + j + " " + k + ": unconstrainedCost="
                            + unconstrainedCost + " constrainedCost=" + constrainedCost + " minCost=" + minimizableCost
    ...
    public static Pipe makeSpacePredictionPipe() {
        Pipe p = new SerialPipes(new Pipe[] { new CharSequence2TokenSequence("."), new TokenSequenceLowercase(),
                new TestMEMMTokenSequenceRemoveSpaces(), new TokenText(), new OffsetConjunctions(true, new int[][] { // {0},
                                                                                                                     // /*{1},{-1,0},{0,1},
                                                                                                                     // */
                        { 1 }, { -1, 0 }, { 0, 1 },
    ...
    public void disabledtestAddOrderNStates() {
        ...
        assertTrue("Final zero-order likelihood <" + lik1 + "> greater than first-order <" + lik2 + ">", lik1 < lik2);
    ...
    public void disabledtestPrint() {
        Pipe p = new SerialPipes(new Pipe[] { new CharSequence2TokenSequence("."), new TokenText(),
                new TestMEMM.TestMEMMTokenSequenceRemoveSpaces(), new TokenSequence2FeatureVectorSequence(),
                new PrintInputAndTarget(), });
    ...
    //
    // /*
    // * TestSuite theSuite; if (args.length > 0) { theSuite = new TestSuite(); for (int i = 0; i < args.length; i++)
    // * { theSuite.addTest (new TestMEMM (args [i])); } } else { theSuite = (TestSuite) suite(); }
    // *
