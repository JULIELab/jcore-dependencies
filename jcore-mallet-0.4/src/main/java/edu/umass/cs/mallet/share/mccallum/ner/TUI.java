...
public class TUI
{
	...
	public static void main (String[] args) throws FileNotFoundException, Exception
	{
		...
		if (useHyperbolicPriorOption.value) {
			crf.setUseHyperbolicPrior (true);
			crf.setHyperbolicPriorSlope (hyperbolicSlopeOption.value);
			crf.setHyperbolicPriorSharpness (hyperbolicSharpnessOption.value);
		} else {
			crf.setGaussianPriorVariance (gaussianVarianceOption.value);
		}
		for (int i = 0; i < crf.numStates(); i++) {
			Transducer.State s = crf.getState (i);
			if (s.getName().charAt(0) == 'I')
				s.setInitialCost (Double.POSITIVE_INFINITY);
		}
		...
		MultiSegmentationEvaluator eval =
			new MultiSegmentationEvaluator (new String[] {"B-PER", "B-LOC", "B-ORG", "B-MISC"},
																			new String[] {"I-PER", "I-LOC", "I-ORG", "I-MISC"});
		eval.setViterbiOutputFilePrefix(viterbiFilePrefixOption.value);
