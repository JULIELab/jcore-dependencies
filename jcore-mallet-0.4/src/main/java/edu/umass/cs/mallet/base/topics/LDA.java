...
public class LDA {
                        ...
                        Random r)
	{
		...
		for (int di = 0; di < numDocs; di++) {
			FeatureSequence fs = (FeatureSequence) ilist.getInstance(di).getData();
			seqLen = fs.getLength();
			numTokens += seqLen;
	...
	private void sampleTopicsForAllDocs (Random r)
	{
		...
		for (int di = 0; di < topics.length; di++) {
			sampleTopicsForOneDoc ((FeatureSequence)ilist.getInstance(di).getData(),
...
*/
  ...
  public void printDocumentTopics (PrintWriter pw)
  {
    ...
    for (int di = 0; di < topics.length; di++) {
      ...
      for (int ti = 0; ti < numTopics; ti++)
        pw.print (((float)docTopicCounts[di][ti])/docLen); pw.print (' ');
      pw.println (ilist.getInstance(di).getSource().toString()); pw.print (' ');
    }
  }
  ...
  // Serialization
	...
	public static void main (String[] args) throws IOException
	{
		...
		LDA lda = new LDA (10);
		lda.estimate (ilist, numIterations, 50, 0, null, new Random());  // should be 1100
