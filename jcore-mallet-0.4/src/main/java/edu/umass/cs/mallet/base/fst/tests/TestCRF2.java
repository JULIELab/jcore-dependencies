...
public class TestCRF2 extends TestCase
{
	...
	public void testGetSetParameters ()
	{
		...
		CRF2 crf = new CRF2 (inputAlphabet, outputAlphabet);
		String[] stateNames = new String[numStates];
		for (int i = 0; i < numStates; i++)
			stateNames[i] = "state"+i;
		crf.addFullyConnectedStates (stateNames);
		CRF2.MinimizableCRF mcrf = crf.getMinimizableCRF (new InstanceList(null));
	...
	public void testCost (int useSave)
	{
		...
		//System.err.println("CRF parameters. hyperbolicPriorSlope: " + crf.getUseHyperbolicPriorSlope() + ". hyperbolicPriorSharpness: " + crf.getUseHyperbolicPriorSharpness() + ". gaussianPriorVariance: " + crf.getGaussianPriorVariance()+ ". defaultFeatureIndex: " + crf.getDefaultFeatureIndex());
		// And read it back in
		crf = null;
		try {
			ObjectInputStream ois = new ObjectInputStream(new FileInputStream(f));
			crf = (CRF2) ois.readObject();
			ois.close();
		}
		catch (IOException e) {
		...
		CRF2.MinimizableCRF mcrf = crf.getMinimizableCRF (ilist);
	...
	public class TestCRFTokenSequenceRemoveSpaces extends Pipe implements Serializable
	{
	...
	public void doTestSpacePrediction (boolean testCostAndGradient, int useSaved)
	{
		Pipe p = new SerialPipes (new Pipe[] {
			new CharSequence2TokenSequence ("."),
			new TokenSequenceLowercase (),
		...
		File f = new File("TestObject.obj");
		File f2 = new File("TestObject2.obj");
		File f3 = new File("TestList.obj");
		InstanceList instances = new InstanceList (p);
		instances.add (new ArrayIterator (data));
		InstanceList[] lists = instances.split (new double[] {.5,.5});
		CRF2 crf = new CRF2 (p.getDataAlphabet(), p.getTargetAlphabet());
		crf.addFullyConnectedStatesForLabels ();
		if (testCostAndGradient) {
			Minimizable.ByGradient minable = crf.getMinimizableCRF (lists[0]);
			TestMinimizable.testCostAndGradient (minable);
			...
			System.out.println ("Training Accuracy after saving = "+crf.averageTokenAccuracy (lists[0]));
			System.out.println ("Testing  Accuracy after saving = "+crf.averageTokenAccuracy (lists[1]));
		}
	}
	...
	public void testSerialization (int useSaved)
	{
		doTestSpacePrediction (false, useSaved);
	}
	
	public static Test suite ()
	{
		return new TestSuite (TestCRF2.class);
	}
	...
	public static void main (String[] args)
	{
		...
		if (args.length >0) {
			TestCRF2 t = new TestCRF2("testTrain");
			if (args[0].equals ("testTrain"))
				t.testTrain();
			else if (args[0].equals ("testSerialization"))
				t.testSerialization(Integer.parseInt(args[1]));
