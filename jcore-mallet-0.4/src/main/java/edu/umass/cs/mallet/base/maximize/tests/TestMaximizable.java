...
public class TestMaximizable extends TestCase
{
	...
	MalletLogger.getLogger(TestMaximizable.class.getName());
	
	public TestMaximizable (String name) {
		super (name);
	}
	...
	testValueAndGradientInDirection (Maximizable.ByGradient maxable,
																	 double[] direction)
	{
		double[] oldParameters = new double [maxable.getNumParameters()];
		double[] parameters = new double [maxable.getNumParameters()];
		...
		double epsilon = 0.1 / MatrixOps.twoNorm(analyticGradient);
		MatrixOps.plusEquals (parameters, direction, epsilon);
		//logger.fine ("Parameters:"); parameters.print();
		maxable.setParameters (parameters);
		double epsValue = maxable.getValue();
		double slope = (epsValue - value) / epsilon;
		double directionGradient = MatrixOps.dotProduct (analyticGradient, direction);
...
//		{ int i = 0;   // Uncomment this line to debug one parameter at a time -cas
			...
			logger.info ("TestMaximizable checking singleIndex "+i+
													...
													", value+epsilon slope = "+slope+
													": slope difference = "+Math.abs(slope - analyticGradient[i]));
...
*/
   ...
   * Tests that getValue and getValueGradient are consistent 
	...
	public static boolean testValueAndGradientRandomParameters 
 	                      (Maximizable.ByGradient maxable, Random r)
	{
	...
	{
		return new TestSuite (TestMaximizable.class);
	}
