...
public class AdaBoostM2Trainer extends ClassifierTrainer
{
		         ...
		         Classifier initialClassifier)
    {
	  ...
	  do {
		...
		Instance inst = trainingInsts.getInstance(sampleIndices[i]);
		roundTrainingInsts.add(inst, 1);
	      }
	      weakLearners[round] = weakLearner.train(roundTrainingInsts, validationList);
	      // Calculate the pseudo-loss of weak learner
	      for (int i = 0; i < trainingInsts.size(); i++) {
		Instance inst = trainingInsts.getInstance(i);
	  ...
	  logger.info("AdaBoostM2Trainer weight[weakLearner[" + i + "]]=" + classifierWeights[i]);
        return new AdaBoostM2 (trainingInsts.getPipe(), weakLearners, classifierWeights);
    }
